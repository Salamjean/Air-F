<header class="flex items-center justify-between px-6 py-4 bg-white shadow-sm border-b border-gray-100">
    <div class="flex items-center">
        ...
        <div class="hidden md:block ml-4">
            ...
            <p class="text-sm text-gray-500">Bienvenue dans votre espace Personnel</p>
        </div>
    </div>
    ...
    <div class="flex items-center space-x-4">
        ...
        <div class="relative group">
            <button class="flex items-center space-x-2 focus:outline-none">
                <img src="https://ui-avatars.com/api/?name={{ Auth::guard('user')->user()->name . ' ' . Auth::guard('user')->user()->prenom ?? 'personnel' }}&background=EBF4FF&color=EF4444"
                    class="w-10 h-10 rounded-full border-2 border-white shadow-sm" alt="personnel">
                <div class="hidden md:block text-left">
                    <span
                        class="block text-sm font-medium text-gray-700">{{ Auth::guard('user')->user()->name . ' ' . Auth::guard('user')->user()->prenom ?? 'personnel' }}</span>
                    <span class="block text-xs text-gray-500">Personnel</span>
                </div>
