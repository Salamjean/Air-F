    ...
    <div class="container mx-auto px-6 py-8">
        <div class="mx-auto max-w-4xl">
            <div class="mb-8 flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-[#111827]">Modifier le Forfait</h1>
                    <p class="text-gray-500 mt-2">Modifiez les informations du forfait <span
                            class="text-red-600 font-bold">{{ $forfait->name }}</span>.</p>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                ...
                <form action="{{ route('admin.forfaits.update', $forfait) }}" method="POST" class="p-8 md:p-10 space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <label for="name" class="block text-sm font-bold text-[#111827] mb-2">Identifiant (ex: F1,
                                F2)</label>
                            <input type="text" name="name" id="name" value="{{ old('name', $forfait->name) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="label" id="label" value="{{ old('label', $forfait->label) }}"
                            class="w-full px-4 py-3 rounded-xl border-gray-300 focus:border-red-600 focus:ring-0 bg-gray-50 font-medium"
                            required>
                    ...
                    <div>
                        ...
                        <div id="tasks-container" class="space-y-3">
                            @forelse($forfait->tasks as $task)
                                <div class="flex items-center gap-3 task-row">
                                    <input type="text" name="tasks[]" value="{{ $task->description }}"
