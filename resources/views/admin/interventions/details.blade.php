...
@section('content')
    <div class="container mx-auto px-6 py-10">
        <!-- Navigation -->
        <div class="mb-8">
            <a href="{{ route('admin.interventions.valider') }}"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                            <span
                                class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-green-50 text-green-700 border border-green-100 text-sm font-bold uppercase tracking-wider">
                        ...
                        @if(in_array($intervention->statut, ['facture', 'accord', 'finance', 'receptionne', 'attente_paiement', 'payer']))
                            <div class="mb-10 bg-gray-50/50 p-6 rounded-3xl border border-gray-100">
                                ...
                                <div class="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0 px-4">
                                    ...
                                    <div class="flex flex-col items-center flex-1">
                                        <div
                                            class="w-8 h-8 rounded-full {{ in_array($intervention->statut, ['finance', 'receptionne', 'attente_paiement', 'payer']) ? 'bg-teal-500 text-white' : 'bg-gray-200 text-gray-400' }} flex items-center justify-center shadow-lg transition-all">
                                            <i class="fas fa-university text-xs"></i>
                                        </div>
                                        <span
                                            class="text-[8px] mt-2 font-bold {{ in_array($intervention->statut, ['finance', 'receptionne', 'attente_paiement', 'payer']) ? 'text-teal-600' : 'text-gray-400' }} uppercase text-center">Réception</span>
                                    </div>

                                    <div
                                        class="hidden md:block w-full h-0.5 {{ in_array($intervention->statut, ['receptionne', 'attente_paiement', 'payer']) ? 'bg-teal-500' : 'bg-gray-200' }} -mt-5">
                                    </div>

                                    <!-- Step 4: Délai -->
                                    <div class="flex flex-col items-center flex-1">
                                        <div
                                            class="w-8 h-8 rounded-full {{ in_array($intervention->statut, ['receptionne', 'attente_paiement', 'payer']) ? 'bg-teal-500 text-white' : 'bg-gray-200 text-gray-400' }} flex items-center justify-center shadow-lg transition-all">
                                            <i class="fas fa-calendar-alt text-xs"></i>
                                        </div>
                                        <span
                                            class="text-[8px] mt-2 font-bold {{ in_array($intervention->statut, ['receptionne', 'attente_paiement', 'payer']) ? 'text-teal-600' : 'text-gray-400' }} uppercase text-center">Délai</span>
                                    </div>
                                    ...
                                    <div class="flex flex-col items-center flex-1">
                                        ...
                                        <span
                                            class="text-[8px] mt-2 font-bold {{ in_array($intervention->statut, ['payer']) ? 'text-green-700' : 'text-gray-400' }} uppercase text-center">Payé</span>
                                    </div>
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        @if($intervention->document_1)
                            ...
                            <div class="bg-blue-50/30 rounded-2xl p-4 border border-blue-100">
                                ...
                                @else
                                    <a href="{{ asset('storage/' . $intervention->document_1) }}" target="_blank"
                                        class="flex items-center gap-3 p-3 bg-white rounded-xl border border-blue-100 hover:shadow-md transition-all group h-full">
                                        <div
                        ...
                        @foreach($intervention->documents as $doc)
                            ...
                            <div class="bg-indigo-50/30 rounded-2xl p-4 border border-indigo-100">
                                <span class="block text-xs uppercase font-bold text-indigo-400 tracking-wider mb-3">Document
                                    Joint</span>
                                @if($isImg)
                                    <a href="{{ asset('storage/' . $doc->file_path) }}" target="_blank" class="block group">
                                        <div class="relative rounded-xl overflow-hidden border border-indigo-200 aspect-video">
                                            <img src="{{ asset('storage/' . $doc->file_path) }}"
                                                class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                            <div
                                                class="absolute inset-0 bg-indigo-900/10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white">
                                                <i class="fas fa-search-plus text-xl"></i>
                                            </div>
                                        </div>
                                    </a>
                                @else
                                    <a href="{{ asset('storage/' . $doc->file_path) }}" target="_blank"
                                        class="flex items-center gap-3 p-3 bg-white rounded-xl border border-indigo-100 hover:shadow-md transition-all group h-full">
                                        <div
                                            class="w-10 h-10 rounded-lg bg-indigo-100 text-indigo-600 flex items-center justify-center group-hover:bg-indigo-600 group-hover:text-white transition-all">
                        ...
                        @if(!$intervention->document_1 && !$intervention->document_2 && $intervention->documents->count() === 0)
                            <div
                                class="col-span-full py-8 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
            ...
            <div class="space-y-8">
                ...
                <div class="bg-gray-900 rounded-3xl shadow-xl p-8 text-white relative overflow-hidden group">
                    ...
                    <p class="text-gray-400 text-sm mb-8 relative z-10">Validez et envoyez cet ordre de mission pour
                        traitement immédiat.</p>
