...
@section('content')
    <div class="container mx-auto px-6 py-10">
        <!-- Navigation -->
        <div class="mb-8">
            <a href="{{ route('admin.interventions.valider') }}" class="inline-flex items-center text-gray-500 hover:text-gray-800 transition-colors font-medium group">
                <div class="w-8 h-8 rounded-full bg-white shadow-sm border border-gray-200 flex items-center justify-center mr-3 group-hover:border-gray-300 group-hover:shadow-md transition-all">
                    <i class="fas fa-arrow-left text-sm group-hover:-translate-x-0.5 transition-transform"></i>
                </div>
                Retour aux interventions validées
            </a>
        </div>
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                            <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-green-50 text-green-700 border border-green-100 text-sm font-bold uppercase tracking-wider">
                                <span class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                                {{ ucfirst($intervention->statut) }}
                            </span>
                            <span class="text-sm font-mono text-black bg-gray-50 px-3 py-1 rounded-lg border border-gray-100">
                                {{ $intervention->code }}
                            </span>
                        </div>
                        ...
                        <div class="mb-8 flex flex-col items-center justify-center p-8 bg-green-50 rounded-3xl border border-green-100 shadow-sm relative overflow-hidden group">
                           <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform duration-500">
                            ...
                            <span class="text-xs font-bold text-green-600 uppercase tracking-widest mb-3 relative z-10">Paiement fixé pour le</span>
                            <div class="flex items-center gap-4 relative z-10">
                                <div class="w-12 h-12 rounded-xl bg-green-500 text-white flex items-center justify-center shadow-lg shadow-green-200">
                            ...
                            @if($intervention->delai_paiement)
                                <div class="mt-4 px-4 py-1.5 bg-white/50 backdrop-blur-sm rounded-full border border-green-200 text-sm font-bold text-green-700 relative z-10">
                        ...
                        <div class="mb-10 bg-gray-50/50 p-6 rounded-3xl border border-gray-100">
                            <h4 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-6 text-center">Évolution du processus de règlement</h4>
                            <div class="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0 px-4">
                                <!-- Step 1: Facturée -->
                                <div class="flex flex-col items-center flex-1">
                                    <div class="w-10 h-10 rounded-full {{ in_array($intervention->statut, ['facture', 'accord', 'finance', 'payer']) ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-400' }} flex items-center justify-center shadow-lg transition-all">
                                        <i class="fas fa-file-invoice"></i>
                                    </div>
                                    <span class="text-[10px] mt-2 font-bold {{ in_array($intervention->statut, ['facture', 'accord', 'finance', 'payer']) ? 'text-green-600' : 'text-gray-400' }} uppercase">Facturation</span>
                                    <span class="text-[8px] text-gray-400 font-medium whitespace-nowrap">Rapport validé</span>
                                </div>

                                <div class="hidden md:block w-full h-1 {{ in_array($intervention->statut, ['accord', 'finance', 'payer']) ? 'bg-green-500' : 'bg-gray-200' }} -mt-6"></div>

                                <!-- Step 2: Accordée -->
                                <div class="flex flex-col items-center flex-1">
                                    <div class="w-10 h-10 rounded-full {{ in_array($intervention->statut, ['accord', 'finance', 'payer']) ? 'bg-indigo-500 text-white' : 'bg-gray-200 text-gray-400' }} {{ $intervention->statut == 'accord' ? 'ring-4 ring-indigo-100 animate-pulse' : '' }} flex items-center justify-center shadow-lg transition-all">
                                        <i class="fas fa-file-signature"></i>
                                    </div>
                                    <span class="text-[10px] mt-2 font-bold {{ in_array($intervention->statut, ['accord', 'finance', 'payer']) ? 'text-indigo-600' : 'text-gray-400' }} uppercase text-center">Accord Admin</span>
                                    <span class="text-[8px] text-gray-400 font-medium whitespace-nowrap">Facture soumise</span>
                                </div>

                                <div class="hidden md:block w-full h-1 {{ in_array($intervention->statut, ['finance', 'payer']) ? 'bg-indigo-500' : 'bg-gray-200' }} -mt-6"></div>

                                <!-- Step 3: Finance -->
                                <div class="flex flex-col items-center flex-1">
                                    <div class="w-10 h-10 rounded-full {{ in_array($intervention->statut, ['finance', 'payer']) ? 'bg-teal-500 text-white' : 'bg-gray-200 text-gray-400' }} {{ $intervention->statut == 'finance' ? 'ring-4 ring-teal-100 animate-pulse' : '' }} flex items-center justify-center shadow-lg transition-all">
                                        <i class="fas fa-university"></i>
                                    </div>
                                    <span class="text-[10px] mt-2 font-bold {{ in_array($intervention->statut, ['finance', 'payer']) ? 'text-teal-600' : 'text-gray-400' }} uppercase text-center">En Finance</span>
                                    <span class="text-[8px] text-gray-400 font-medium whitespace-nowrap">En attente paiement</span>
                                </div>

                                <div class="hidden md:block w-full h-1 {{ in_array($intervention->statut, ['payer']) ? 'bg-teal-500' : 'bg-gray-200' }} -mt-6"></div>

                                <!-- Step 4: Payée -->
                                <div class="flex flex-col items-center flex-1">
                                    <div class="w-10 h-10 rounded-full {{ in_array($intervention->statut, ['payer']) ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-400' }} {{ $intervention->statut == 'payer' ? 'ring-4 ring-green-100' : '' }} flex items-center justify-center shadow-lg transition-all">
                                        <i class="fas fa-check-double"></i>
                                    </div>
                                    <span class="text-[10px] mt-2 font-bold {{ in_array($intervention->statut, ['payer']) ? 'text-green-700' : 'text-gray-400' }} uppercase text-center">Payé</span>
                                    <span class="text-[8px] text-gray-400 font-medium whitespace-nowrap underline italic">{{ $intervention->date_paiement_prevue ? \Carbon\Carbon::parse($intervention->date_paiement_prevue)->format('d/m/Y') : 'À planifier' }}</span>
                                </div>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="flex flex-wrap items-center gap-4 text-gray-500">
                            ...
                            @if($intervention->site)
                                <span class="inline-flex items-center gap-2 text-sm font-medium text-gray-600 bg-gray-100 px-3 py-1 rounded-lg border border-gray-200">
                                    <i class="fas fa-map-marker-alt text-red-500"></i>
                                    {{ $intervention->site->name }} ({{ $intervention->site->code }})
                                </span>
                            @endif
                            <span class="inline-flex items-center gap-2 text-sm font-bold text-gray-900 bg-amber-50 px-3 py-1 rounded-lg border border-amber-200">
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="bg-blue-50 rounded-2xl p-5 border border-blue-100 flex items-center gap-4">
                            <div class="w-12 h-12 rounded-xl bg-blue-500 text-white flex items-center justify-center shadow-lg shadow-blue-200">
                                <i class="far fa-calendar-alt text-xl"></i>
                            </div>
                            <div>
                                <span class="block text-xs uppercase font-bold text-blue-400 tracking-wider mb-1">Date de début</span>
                        ...
                        <div class="bg-indigo-50 rounded-2xl p-5 border border-indigo-100 flex items-center gap-4">
                            <div class="w-12 h-12 rounded-xl bg-indigo-500 text-white flex items-center justify-center shadow-lg shadow-indigo-200">
                                <i class="far fa-calendar-check text-xl"></i>
                            </div>
                            <div>
                                <span class="block text-xs uppercase font-bold text-indigo-400 tracking-wider mb-1">Date de fin</span>
                                <span class="block text-lg font-bold text-gray-900">
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                        ...
                        <div class="px-4 py-2 bg-red-50 text-red-600 rounded-xl border border-red-100 font-bold text-sm">
                            {{ number_format($intervention->montant, 0, ',', ' ') }} € H.T par agent
                        </div>
                    </div>

                    <div class="bg-gray-50 rounded-2xl p-6 border border-gray-100">
                        <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">Interventions incluses dans ce forfait :</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach($intervention->forfaitTasks as $task)
                                <div class="flex items-start gap-3 p-3 bg-white rounded-xl border border-gray-100">
                                    <div class="mt-1 w-5 h-5 rounded-full bg-green-100 text-green-600 flex items-center justify-center flex-shrink-0">
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div class="bg-slate-50 rounded-2xl p-5 border border-slate-100 flex items-center gap-4">
                            <div class="w-12 h-12 rounded-xl bg-slate-600 text-white flex items-center justify-center shadow-lg">
                                <i class="fas fa-clock text-xl"></i>
                            </div>
                            <div>
                                <span class="block text-xs uppercase font-bold text-slate-400 tracking-wider mb-1">Début Réel</span>
                                <span class="block text-lg font-bold text-gray-900">
                        ...
                        <div class="bg-slate-50 rounded-2xl p-5 border border-slate-100 flex items-center gap-4">
                            ...
                            <div>
                                <span class="block text-xs uppercase font-bold text-slate-400 tracking-wider mb-1">Fin Réelle</span>
                                <span class="block text-lg font-bold text-gray-900">
                    ...
                    @if($intervention->rapport_commentaire)
                        <div class="mb-8">
                            <span class="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-3 text-center md:text-left">Observations du Personnel</span>
                            <div class="bg-red-50/50 p-6 rounded-2xl border border-red-100 text-gray-700 italic leading-relaxed">
                    ...
                    @if($intervention->rapport_path)
                        <div>
                            <span class="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-3">Document Joint</span>
                            @php
                            ...
                            @if($isImage)
                                <div class="relative group max-w-md mx-auto md:mx-0">
                                    <img src="{{ asset('storage/' . $intervention->rapport_path) }}" class="rounded-2xl shadow-lg border border-gray-200 cursor-pointer hover:opacity-95 transition-all" alt="Rapport photo">
                                    <a href="{{ asset('storage/' . $intervention->rapport_path) }}" target="_blank" class="absolute bottom-4 right-4 bg-white/90 backdrop-blur-sm text-gray-900 p-3 rounded-xl shadow-lg opacity-0 group-hover:opacity-100 transition-all flex items-center gap-2 text-sm font-bold">
                                        <i class="fas fa-search-plus"></i> Agrandir
                                    </a>
                                </div>
                            @else
                                <a href="{{ asset('storage/' . $intervention->rapport_path) }}" target="_blank" class="inline-flex items-center gap-4 p-5 bg-gray-50 rounded-2xl border border-gray-100 hover:bg-white hover:shadow-md transition-all group w-full md:w-auto">
                                    <div class="w-12 h-12 rounded-xl bg-red-100 text-red-600 flex items-center justify-center group-hover:bg-red-600 group-hover:text-white transition-all shadow-sm">
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @foreach($intervention->equipements as $equip)
                            <div class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-white hover:shadow-sm transition-all">
                                ...
                                <div class="flex-grow">
                                    ...
                                    <span class="text-xs font-bold text-red-600 uppercase">{{ $equip->pivot->quantity }} {{ $equip->unit }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="space-y-8">
                <!-- Action Card -->
                 <div class="bg-gray-900 rounded-3xl shadow-xl p-8 text-white relative overflow-hidden group">
                    <div class="absolute top-0 right-0 w-64 h-64 bg-gray-800 rounded-full blur-3xl opacity-20 -translate-y-1/2 translate-x-1/2"></div>
                    
                    <h3 class="text-xl font-bold mb-2 relative z-10">Actions Requises</h3>
                    <p class="text-gray-400 text-sm mb-8 relative z-10">Validez et envoyez cet ordre de mission pour traitement immédiat.</p>
                    
                    @if($intervention->statut === 'valider')
                        <button onclick="confirmEnvoyerDetails('{{ $intervention->id }}')" 
                            ...
                            <span>Envoyer l'ordre</span>
                            <i class="fas fa-paper-plane text-red-500 animate-pulse"></i>
                        </button>
                        
                        <div class="my-4 border-t border-gray-700/50 relative z-10"></div>
                        
                        <button onclick="openDirectConfirmationModal('{{ $intervention->id }}')" 
                            class="w-full py-3 px-6 bg-green-500 hover:bg-green-600 text-white rounded-xl font-bold transition-all shadow-lg flex items-center justify-center gap-3 relative z-10">
                            <span>Confirmer Directement</span>
                            <i class="fas fa-check-double text-white"></i>
                        </button>
                    @else
                        <button disabled
                            class="w-full py-4 px-6 bg-gray-200 text-gray-400 rounded-xl font-bold cursor-not-allowed flex items-center justify-center gap-3 relative z-10">
                            <span>Envoyer l'ordre</span>
                    ...
                    <div class="p-6 space-y-8">
                        ...
                        <div>
                             ...
                             <div class="flex items-start gap-4">
                                <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 text-white flex items-center justify-center shadow-md">
                        ...
                        <div>
                             ...
                             @if($intervention->personnels->count() > 0)
                                ...
                                @endforeach
                             @else
                                <div class="p-4 bg-yellow-50 text-yellow-800 rounded-xl border border-yellow-100 flex items-center gap-3">
    ...
    <script>
        function confirmEnvoyerDetails(id) {
            Swal.fire({
                title: 'Confirmer l\'envoi ?',
                text: "Cette action est irréversible et notifiera les parties concernées.",
                icon: 'warning',
                iconColor: '#10B981',
