    ...
    <div class="container mx-auto px-6 py-10">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-10 gap-6">
            ...
            <div class="w-full md:w-auto relative">
                <form action="{{ route('prestataire.interventions.historique') }}" method="GET" class="relative group">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-200 group">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex -space-x-2">
                                        @foreach ($intervention->personnels as $person)
                                            <div class="w-8 h-8 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center text-slate-600 text-[10px] font-bold"
                                                title="{{ $person->name }} {{ $person->prenom }}">
                                                {{ substr($person->name, 0, 1) }}{{ substr($person->prenom, 0, 1) }}
                                            </div>
                                        @endforeach
                                    </div>
                                </td>
